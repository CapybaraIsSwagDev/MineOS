...
local sPath = parentShell and parentShell.path() or ".:/MineOs"
...
local function createShellEnv(dir)
    local env = { shell = shell, multishell = nil }
...
local require
do
    local env = setmetatable(createShellEnv("/MineOs/programs"), { __index = _ENV })
    require = env.require
end
...
-- `execute("echo", "b c")` runs `echo` with a single argument `b c`.
--
...
-- @see execute Run a program directly without parsing the arguments.
...
if #tArgs > 0 then
    ...
    while not bExit do
        ...
        local spli = split(result," ")
        local A = table.concat(spli, " ",2,#spli)
        local B = split(result,"/")
        if result == "" then 
        
        elseif spli[1] == "multishell" then
